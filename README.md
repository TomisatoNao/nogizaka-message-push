...
- **网页管理端** — 浏览器完成日常配置和运维：通道开关、轮询节奏、账号池、监控成员、官方 Bot 增删改，凭证直接填写（写入 `.env` 并自动轮换），实时日志查看，一键重启主程序；保存即校验 + 热重载（详见下方「网页管理端」）
...
│   ├── webui.py             # 网页管理端：配置编辑 / 凭证写入 / 日志查看 / 重启（stdlib http.server）
│   ├── webui_static/
