...
flowchart TD
    ...
    subgraph S4["📢 解耦推送通道 (Pub/Sub)"]
        Push1["🐧 QQ 群 (NapCat OneBot11)"]
        Push2["✈️ Telegram 频道 (HTML / MediaGroup)"]
        Push3["🤖 QQ 官方 Bot (个人 / 群聊 / 指令交互)"]
    end
...
- [🔑 用户与权限管理体系 / User & Auth System](#用户与权限管理体系-user-auth-system)
  ...
  - [2.4 命令行管理工具 (tools/manage_users.py)](#24-命令行管理工具-toolsmanageuserspy)
- [🧩 系统核心概念与架构 / Architecture](#系统核心概念与架构-architecture)
  - [3.1 监控源与推送通道彻底解耦（Pub/Sub 订阅分发架构）](#31-监控源与推送通道彻底解耦pubsub-订阅分发架构)
  - [3.2 双模式认证（Web vs Mobile）与全自动握手续期](#32-双模式认证web-vs-mobile与全自动握手续期)
  - [3.3 配置文件三层加载体系与热重载机制](#33-配置文件三层加载体系与热重载机制)
- [🛠️ 博客监控、智能解析与双语阅读器](#博客监控智能解析与双语阅读器)
...
### 5. 翻译来源透明化
- **AI 模型实时动态标记**：详情页右上角动态显示当前译文生成的 AI 模型名称（如 `Gemini 2.5 Flash` / `GLM-4-Flash` 等），仅在存在译文且处于「日中对照」或「中文」视图时优雅展示。
...
- **多平台广泛适配**：深度支持乃木坂46、樱坂46、日向坂46官方 Web/Mobile 及新版 **Yodel** 客户端。
...
flowchart TD
    ...
    subgraph S["📢 推送通道消费层 (Subscribers)"]
        C1["🐧 NapCat QQ 群 (如 群 533072575)<br/>• 订阅: 消息 / 𝕏 / IG / TikTok / Live<br/>• 过滤: 仅接收 冨里奈央"]
        C2["✈️ Telegram 频道 (如 频道 -100xxx)<br/>• 订阅: 全量开启 (Message + 博客 + 社媒 + 直播)<br/>• 过滤: 全量接收 (留空)"]
        C3["🤖 QQ 官方 Bot (Bot 1 / Bot 2)<br/>• 订阅: Message + 博客<br/>• 过滤: 乃木坂46 博客 + 指定成员"]
    end
...
### 3.2 多平台双模式认证（Web vs Mobile）与全自动握手续期
