...
```
                ...
                ▼                                          ▼                                         ▼
 ┌──────────────────────────────┐           ┌──────────────────────────────┐          ┌──────────────────────────────┐
 │  Phase 1: Message 监控抓取   │           │  Phase 2: 官方博客监控抓取   │          │  Phase 3: 社交媒体与直播录制 │
 │  (乃木坂46 / 日向坂46 / 櫻坂46)│           │  (乃木坂46 / 日向坂46 / 櫻坂46)│          │  (X / IG / TikTok / Live)    │
 └──────────────┬───────────────┘           └──────────────┬───────────────┘          └──────────────┬───────────────┘
                │                                          │                                         │
                ▼                                          ▼                                         ▼
 ┌──────────────────────────────┐           ┌──────────────────────────────┐          ┌──────────────────────────────┐
 ...
 └──────────────┬───────────────┘           └──────────────┬───────────────┘          └──────────────┬───────────────┘
                ...
                ▼                                          ▼                                         ▼
 ┌──────────────────────────────┐           ┌──────────────────────────────┐          ┌──────────────────────────────┐
 │  1.2 并发抓取与滑动窗口去重  │           │  2.2 高清原图并发下载与本地化│          │  3.2 TikTok Live 8s 探测录制 │
 │  · asyncio.Semaphore 限制并发│           │  · data/blog_images/{团}/{人} │          │  · 120B 轻量探测 / ffmpeg 混流│
 │  · 账号与成员解耦独立调度    │           │  · 路径绝对化 + 格式归一化   │          │  · 30min 自动切片 + 断流重连 │
 └──────────────┬───────────────┘           └──────────────┬───────────────┘          └──────────────┬───────────────┘
                │                                          │                                         │
                ▼                                          ▼                                         ▼
 ┌──────────────────────────────┐           ┌──────────────────────────────┐          ┌──────────────────────────────┐
 │  1.3 双引擎 AI 智能轮流翻译  │           │  2.3 DOM 智能分段与图片保护  │          │  3.3 多媒体清洗与 Alt 描述提取│
 │  · Gemini + 智谱 轮流分发    │           │  · 空 <p>/空行切分视效大段落 │          │  · X ?name=orig 原图升级     │
 │  · 偶像语境/假名爱称硬性约束 │           │  · 抽离 <img> 原位保持       │          │  · 图片无障碍 Alt 提取与翻译 │
 └──────────────┬───────────────┘           └──────────────┬───────────────┘          └──────────────┬───────────────┘
                │                                          │                                         │
                ├──────────────────────────────────────────┴─────────────────────────────────────────┤
                ▼                                                                                    ▼
 ┌──────────────────────────────────────────────────────────────┐             ┌──────────────────────────────────────────────┐
 │                  全通道格式化多路由消息推送                  │             │          本地持久化、媒体归档与 SQLite 库    │
 │ · QQ 群 (NapCat / Lagrange OneBot11 协议)                    │             │ · Message 归档: JSON 分片 + 媒体文件         │
 │ · Telegram 频道 / 讨论组 (原生 HTML + MediaGroup)            │             │ · 博客数据库: data/archive/blogs.db          │
 │ · QQ 官方 Bot (个人 C2C / 群聊 / 私聊指令监听交互)           │             │ · 社媒状态库: data/social_state.db           │
 └──────────────────────────────────────────────────────────────┘             └──────────────────────────────────────────────┘
                                                                                             ▲
                                                                                             │
                                                                                             ▼
...
- [🌐 社交媒体监控与直播录制 (X / Instagram / TikTok / Live)](#-社交媒体监控与直播录制-x--instagram--tiktok--live)
  - [6.1 多平台免登录抗封控抓取架构](#61-多平台免登录抗封控抓取架构)
  - [6.2 成员与 Message 账号解耦机制](#62-成员与-message-账号解耦机制)
  - [6.3 TikTok Live 直播探测与 ffmpeg 切片录制](#63-tiktok-live-直播探测与-ffmpeg-切片录制)
  - [6.4 社交媒体与 AI 翻译及多通道推送对接](#64-社交媒体与-ai-翻译及多通道推送对接)
- [🖥️ 网页管理端与日常运维](#️-网页管理端与日常运维)
  - [7.1 七大管理页签功能全览](#71-七大管理页签功能全览)
  - [7.2 QQ 官方机器人私聊交互指令](#72-qq-官方机器人私聊交互指令)
- [⚙️ 服务化部署与运维托管](#️-服务化部署与运维托管)
  - [8.1 Windows 计划任务后台守护（防孤儿进程）](#81-windows-计划任务后台守护防孤儿进程)
  - [8.2 Linux Systemd 用户级/系统级服务化托管](#82-linux-systemd-用户级系统级服务化托管)
  - [8.3 每日健康摘要（死人开关）与数据备份清单](#83-每日健康摘要死人开关与数据备份清单)
- [🔍 常见故障排查 / FAQ](#-常见故障排查--faq)
- [📖 附录与配置参考](#-附录与配置参考)
  - [10.1 全量配置项手册 (config.json)](#101-全量配置项手册-configjson)
  - [10.2 环境变量速查 (.env)](#102-环境变量速查-env)
  - [10.3 命令行工具矩阵 (tools/)](#103-命令行工具矩阵-tools)
  - [10.4 现役成员 ID 速查表](#104-现役成员-id-速查表)
...
### 3.1 监控源与推送通道彻底解耦（Pub/Sub 订阅分发架构）

```text
┌────────────────────────────────────────────────────────┐
│               📡 监控数据源层 (Producers)               │
│  · Message 成员消息 (富里奈央, 金村美玖...)             │
│  · 官方博客增量 (乃木坂46, 樱坂46, 日向坂46)           │
│  · 社交平台动态 (𝕏 / Instagram / TikTok / 直播录制)    │
└───────────────────────────┬────────────────────────────┘
                            │ Pub/Sub 事件总线分发
                            ▼
┌────────────────────────────────────────────────────────┐
│               📢 推送通道消费层 (Subscribers)           │
│  各通道完全独立配置订阅规则与精细白名单过滤：          │
│                                                        │
│  [1] NapCat QQ 群 (群 533072575)                       │
│      · 订阅类型: [✓]Msg  [ ]博客  [✓]𝕏  [✓]IG  [✓]TT  [✓]Live│
│      · 白名单: 仅收 冨里奈央 消息与社媒                │
│                                                        │
│  [2] Telegram 频道 (群 -100xxx)                        │
│      · 订阅类型: [✓]Msg  [✓]博客  [✓]𝕏  [✓]IG  [✓]TT  [✓]Live│
│      · 白名单: 全量接收 (留空)                         │
│                                                        │
│  [3] QQ 官方机器人 (Bot 1 / Bot 2)                     │
│      · 订阅类型: [✓]Msg  [✓]博客  [ ]𝕏  [ ]IG  [ ]TT  [ ]Live│
│      · 白名单: 仅收 乃木坂46 博客与指定成员            │
└────────────────────────────────────────────────────────┘
```
...
### 3.2 双模式认证（Web vs Mobile）与全自动握手续期
...
大家中午好！今天真的超级热呢〜☀️
```
- **双引擎智能轮流调度**：系统在 Google Gemini（3.7/3.6/3.5 Flash）与智谱清言（GLM-4-Flash）之间交替轮流翻译，徽章自动对应本次调用的模型名称；
...
## 🌐 社交媒体监控与直播录制 (X / Instagram / TikTok / Live)
...
### 6.3 TikTok Live 直播探测与 ffmpeg 切片录制
...
```
┌────────┬─────────────┬──────────┬──────────┬──────────┬─────────┬──────────┐
│ 📊状态 │ 👥账号与成员 │ 📢推送通道│ 🌐动态监控│ ⚙️系统设置│ 🔑用户   │ 🛠️高级   │
└────────┴─────────────┴──────────┴──────────┴──────────┴─────────┴──────────┘
```
...
| **👥 账号与成员** | 账号池增删改、凭证即时粘贴更新；**成员与社媒账号一站式绑定（直接配置 X / IG / TikTok / Live 用户名）**；支持成员与账号解耦（选择「(无)」纯社媒监控）。 |
...
### 8.2 Linux Systemd 用户级/系统级服务化托管
