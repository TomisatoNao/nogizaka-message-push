...
```
┌────────┬─────────────┬──────────┬──────────┬──────────┬─────────┬──────────┐
│ 📊状态 │ 👥账号与成员 │ 📢推送通道│ 🌐社交监控│ ⚙️系统设置│ 🔑用户   │ 🛠️高级   │
└────────┴─────────────┴──────────┴──────────┴──────────┴─────────┴──────────┘
```
...
| **👥 账号与成员** | 账号池增删改、凭证即时粘贴更新；支持成员与账号解耦（选择「无-纯社媒/博客」）；从官方 API 一键拉取现役成员目录。 |
| **📢 推送通道** | 集中管控 NapCat QQ、Telegram、QQ 官方 Bot 等通道开关、API 接口、推送路由与独立成员/博客过滤。 |
| **🌐 社交监控** | 𝕏 (Twitter)、Instagram、TikTok 短视频与 TikTok Live 直播录制的可视化开关、轮询间隔设置与 Session 凭证填报。 |
| **⚙️ 系统设置** | 日间/深夜/休眠时段轮询间隔配置、博客爬虫抓取开关、Gemini/智谱 API 参数及图片打标设置。 |
