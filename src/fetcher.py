...
from src.logger import error_logger, log_all, log_response, log_warn
        ...
        for attempt in range(1, MAX_FETCH_ATTEMPTS + 1):
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # 改进 3：只有真正新消息时才写 response_debug.log
                    truly_new = [
                        m for m in new_msgs
                        if str(m.get("id") or m.get("updated_at", "")) not in id_set
                    ]
                    if truly_new:
                        log_response(resp.text, member_name=m_name)  # 改进 2：传入成员名
                ...
                # ---- 401 续期 ----
                    ...
                    log_response(resp.text, member_name=m_name)  # 改进 2
                    log_all(
